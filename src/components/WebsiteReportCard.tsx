...
function ReportContactForm({ websiteUrl, score }: { websiteUrl: string; score: number }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // fallback — submit optimistically
    }
    ...
    <div className="bg-gradient-to-r from-blue-950 to-blue-900 rounded-2xl p-6 sm:p-8 text-white">
      ...
      <form onSubmit={handleSubmit} className="max-w-2xl mx-auto space-y-4 relative">
        ...
        <div className="grid sm:grid-cols-2 gap-3">
          ...
          <div>
            <label htmlFor="rc-email" className="block text-xs font-semibold text-blue-200 mb-1.5">
          ...
          <div>
            <label htmlFor="rc-phone" className="block text-xs font-semibold text-blue-200 mb-1.5">
              Phone
            </label>
            <input
              id="rc-phone"
        ...
        <div className="flex flex-col sm:flex-row items-center gap-3 pt-2">
          ...
          <button
            type="submit"
            disabled={sending}
            className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-8 py-3.5 rounded-full bg-orange-500 text-white font-bold text-sm hover:bg-orange-600 transition-all shadow-lg disabled:opacity-70 disabled:cursor-not-allowed"
          >
            {sending ? "Sending..." : "Get My Free Fix Plan"}
            {!sending && <ArrowRight className="w-4 h-4" />}
          </button>
          <span className="text-xs text-blue-300">
            or call{" "}
            <a href="tel:5592823075" className="text-white font-semibold hover:underline">
              (559) 282-3075
            </a>
          </span>
        </div>
